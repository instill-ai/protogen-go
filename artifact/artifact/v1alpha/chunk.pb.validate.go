...
func (m *SimilarityChunk) validate(all bool) error {
	...
	if all {
		switch v := interface{}(m.GetChunkMetadata()).(type) {
		case interface{ ValidateAll() error }:
			if err := v.ValidateAll(); err != nil {
				errors = append(errors, SimilarityChunkValidationError{
					field:  "ChunkMetadata",
					reason: "embedded message failed validation",
					cause:  err,
				})
	...
	} else if v, ok := interface{}(m.GetChunkMetadata()).(interface{ Validate() error }); ok {
		if err := v.Validate(); err != nil {
			return SimilarityChunkValidationError{
				field:  "ChunkMetadata",
				reason: "embedded message failed validation",
				cause:  err,
			}
