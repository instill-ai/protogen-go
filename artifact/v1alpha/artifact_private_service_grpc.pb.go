...
func (c *artifactPrivateServiceClient) EntityHopAdmin(ctx context.Context, in *EntityHopAdminRequest, opts ...grpc.CallOption) (*EntityHopAdminResponse, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(EntityHopAdminResponse)
	err := c.cc.Invoke(ctx, ArtifactPrivateService_EntityHopAdmin_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func _ArtifactPrivateService_EntityHopAdmin_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(EntityHopAdminRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(ArtifactPrivateServiceServer).EntityHopAdmin(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: ArtifactPrivateService_EntityHopAdmin_FullMethodName,
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(ArtifactPrivateServiceServer).EntityHopAdmin(ctx, req.(*EntityHopAdminRequest))
	}
	return interceptor(ctx, in, info, handler)
}
